...
?>
        ...
        <?php echo form_open_multipart('dashboard/mytest/',array('class'=>"form-horizontal",'role'=>"form" ))?>
                ...
                <div class="panel panel-default <?php echo $cc; ?>">
                    ...
                    <form name="<?php echo $record->id ?>" method="post">
                        ...
                        <div class="col-xs-12" style="margin-bottom: 10px;">
                          ...
                          <input type="text" class="form-control" step="any" value="0" id="total<?php echo $record->id ?>" name="total<?php echo $record->id ?>" readonly>
                          </div>
...
id="operation<?php echo $operation->id ?>" name="operation[<?php echo $operation->id ?>]" 
class="<?php echo $record->id ?>" /></td> 
                <td><?php echo $operation->code ?></td>            
            <td><?php echo $operation->operation ?> </td>
            <td><?php echo $operation->price ?> </td>  
            
            <td>  <!--<input name="txtEmmail" class="val2"/> -->
            
     <input type="text" id="dis_<?php echo $operation->id ?><?php echo $record->id ?>" name="dis_<?php echo $operation->id ?><?php echo $record->id ?>" 
     onkeyup="return last(<?php echo $record->id ?>,<?php echo $operation->id ?>,<?php echo $operation->price ?>);" 
     class="form-control input-sm  dis_<?php echo $record->id ?>" onkeypress="return isNumberKey(event)" value="0" rel="3" readonly />
...
function total_sum(id,op,price){ 
   ...
   //    alert($("operation"+op).val);
    
    $(put4).change(function() {
    							
           if(! $(this).prop('checked')){
               // $(put3).val('');
                $(put3).attr("readonly", true);
                }
           else
                $(put3).attr("readonly", false);
    });
     ...
     $(put2).val(function() {
      ...
      $(put1).each(function() {
                ...
                //$(clas).val(0);
    /*            
                if($(clas).val() <= 100 && $(clas).val() > 0){
                    sum2 = sum - price;
                    dis = price - parseFloat( ( price * $(clas).val() ) /100 );
                    sum2 += dis;
                    //$(put2).val(sum2);
                }
          */      
...
function last(id,op,price){
    var input = "#dis_"+op+id;
    var input2 = "#total"+id;
    ...
    if($(input).val() <= 100 && $(input).val() > 0){
        sum2 = sum3 - price;
        dis = price - parseFloat( ( price * $(input).val() ) /100 );
        sum2 += dis;
        $(input2).val(sum2);
    }
    else{
        alert("لا يمكن أن تكون قيمة الخصم أكبر من 100% أو تساوي صفر");
        */

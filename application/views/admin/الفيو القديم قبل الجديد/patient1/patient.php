        ...
        <fieldset>
            ...
            <legend data-sr="wait 0.3s, then enter left and hustle 100%">البايانات الشخصية</legend>
                ...
                <i class="fa fa-barcode"></i>
                    ...
                    <input type="text" id="code"  name="code" onkeypress="return isNumberKey(event)"  placeholder="رقم الملف" class="form-control text-right" required >
            ...
            <label for="inputUser" class="col-lg-2 control-label">تاريخ الميلاد:</label>
                ...
                <!--div class="col-lg-4">
		      	<div class="bfh-datepicker"  data-format="d-m-y" data-max="today" data-name="data_row"></div>
		      </div-->
       ...
       <div class="panel-group" style="margin-bottom: 3px;" id="accordion">
        ...
        <div class="panel panel-default <?php echo $cc; ?>">
                    ...
                    echo '<div class="col-xs-12 alert-info">
                          ...
                          <input type="text" class="form-control text-center" onkeyup="return discount('.$start.','.$end.','.$t.');" onkeypress="return isNumberKey(event)" name="percentage'.$t.'" id="percentage'.$t.'" style="height:10px;width:100px;" placeholder="%" readonly /> 
                          </div>
                          ...
                          <div calss="col-xs-3"  style="width:15%;float: right; ">
                          <input type="text" class="form-control text-center" onkeyup="return paid('.$end.','.$t.');" onkeypress="return isNumberKey(event)" name="payment'.$t.'" id="payment'.$t.'" style="height:10px;width:100px;" readonly /> 
                          </div>
     ...
     <table  class="table table-bordered" role="table">
        ...
        <tbody>
            ...
            for($b = 0 ; $b < count($records5) ; $b++){
                $val = 0;
                if($records5[key($records5)][0]->status == 0){
                    for($a = 0 ; $a < count($records5[key($records5)]) ; $a++)
                        $val += $records5[key($records5)][$a]->paid;
                }
                else{
                    $DB1 = $this->load->database('kingdom', TRUE);
                    $h = $DB1->get_where('payment',array('patient_id'=>$records5[key($records5)][0]->p_id,'hospital_id'=>1,'out_date'=>date("Y-m-d")));
                    $pay = $h->row_array();
                    $val = $pay['paid'];
                }
                ...
                echo '<tr>
                      <td>'.($b+1).'</td>
                      <td>'.$records5[key($records5)][0]->a_name.'</td>
                      <td>'.$records5[key($records5)][0]->mobile.'</td>
